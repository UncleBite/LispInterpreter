        ...
        if (node.a.val.string_value == "+"){
            if(node.children.empty()){
                ...
                for (int i = 0; i<node.children.size(); i++){
                    ...
                    else{throw InterpreterSemanticError("Error: children is wrong type");}
                }
...
{
    auto value = parseExpression(node);
    if (value.isType(Boolean))
    {
        if(env.boolean_allow(identifier))
        {
            env.add_boolean_map(identifier, value.val.bool_value);
            return value;
        }
    ...
    {
        if(env.number_allow(identifier)){
            env.add_number_map(identifier, value.val.num_value);
            return value;
        }
        throwError("Error: identifier already defined");
    } else if(value.isIdentifier()){
        if (env.number_map.find(value.val.string_value) != env.number_map.end()) //find the number
        {
            auto val = env.return_number(value.val.string_value);
            env.add_number_map(identifier, val);
            return Expression(val);
        } else if(env.boolean_map.find(value.val.string_value)!= env.boolean_map.end())
        {
            auto val = env.return_boolean(value.val.string_value);
            env.add_boolean_map(identifier, val);
            return Expression(val);
        }
    }
    return Expression();
